...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!isset($_SESSION["user_id"])) {
    header("Location: " . APP_URL . "login.php?redirect=" . urlencode("product.php?id=" . $id));
    exit;
  }
...
/**
 * If your DB stores cover_image like "images/x.png" (relative),
 * this will make it absolute with APP_URL.
 * If it already starts with http(s) or APP_URL, it keeps it.
 */
function asset_url(string $path): string {
  $path = trim($path);
  if ($path === "") return "";
  if (preg_match('~^https?://~i', $path)) return $path;
  if (defined('APP_URL') && str_starts_with($path, APP_URL)) return $path;
...
<head>
  ...
  <title><?= h($book['title']) ?> - Book Webshop</title>
  ...
  <div class="container">
    ...
    <section class="product-shell">
      ...
      <div class="product-media">
        ...
        <a class="back-to-catalog" href="<?= APP_URL ?>catalog.php">← Back to catalog</a>

        <div class="product-cover">
          <img
            src="<?= h(asset_url((string)$book['cover_image'])) ?>"
            alt="<?= h($book['title']) ?>"
          />
        </div>
      ...
      <div class="product-content">
        ...
        <div class="product-head">
          <h1 class="product-title"><?= h(ucwords($book['title'])) ?></h1>
          <p class="product-author">by <?= h($book['author']) ?></p>
      ...
      <div class="rec-grid">
        <?php foreach ($recs as $r): ?>
          ...
          <article class="rec-card">
            <a href="<?= APP_URL ?>product.php?id=<?= (int)$r['id'] ?>" class="rec-link">
              <div class="rec-img">
                <img src="<?= h(asset_url((string)$r['cover_image'])) ?>" alt="<?= h($r['title']) ?>">
              </div>
              <div class="rec-info">
                <h3><?= h(ucwords($r['title'])) ?></h3>
                <p class="rec-author"><?= h($r['author']) ?></p>
                <p class="rec-price">
...
(function(){
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      const res = await fetch('<?= APP_URL ?>ajax/review_save.php', {
        method: 'POST',
        body: fd
      });
